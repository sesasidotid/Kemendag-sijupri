...
class UserPendidikanService extends UserPendidikan
{
    use SearchService;
    
    private const bucket_name = "pendidikan";
    ...
    public function findById($id): ?UserPendidikanService
    {
        return $this
            ->where('id', $id)
            ->where('delete_flag', false)
    ...
    public function findLatestByNip($nip): ?UserPendidikanService
    {
        return $this
            ->where('nip', $nip)
            ->where('delete_flag', false)
            ->where('task_status', '!=', TaskStatus::REJECT)
            ->orderBy('bulan_kelulusan', 'desc')
            ->first();
    }
