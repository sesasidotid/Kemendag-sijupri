...
@section('content')
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('/maintenance/kota/update', ['id' => $kota->id]) }}" class="mb-3">
                    @csrf
                    @method('PUT')
                    <div class="col-lg-12">
                        <div class="mb-3">
                            ...
                            <select class="form-select mb-3" name="provinsi_id" aria-label="Provinsi" id="Provinsi">
                                <option {{ $kota->provinsi_id ? '' : 'selected' }}>Pilih Provinsi</option>
                                @if (isset($provinsiList))
                                    @foreach ($provinsiList as $item)
                                        <option {{ $kota->provinsi_id == $item->id ? 'selected' : '' }}
                                            value="{{ $item->id }}">
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                    ...
                    <div class="col-lg-12">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="name" id="Nama"
                                placeholder="Masukkan Nama" value="{{ $kota->name }}">
                        </div>
                    </div>
                    <div class="col-lg-12">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="description" id="Deskripsi"
                                placeholder="Masukkan Deskripsi" value="{{ $kota->description }}">
                        </div>
                    </div>
                    <input type="hidden" name="latitude" id="latitude" value="{{ $kota->latitude }}">
                    <input type="hidden" name="longitude" id="longitude" value="{{ $kota->longitude }}">
                    <div class="col-lg-12">
